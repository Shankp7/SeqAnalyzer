...
* **True Stream Parsing:** Files are read line-by-line iteratively. The $O(1)$ memory constraint guarantees the application will never crash or spike RAM regardless of the dataset size.
* **Batch Processing:** Allows users to ingest an entire directory of sequences, automating the parsing and database logging of hundreds of files sequentially without manual intervention.
...
* **Remote NCBI Fetching:** Directly integrates with `Bio.Entrez` to securely fetch genome sequences over the network via their NCBI Accession ID (e.g., NC_002549.1), bypassing the need to download files via a web browser.

### 3. Local History Tracking (SQLite3)
* **Lightweight Storage:** Uses an embedded SQLite3 database to log historical analysis runs securely on your local machine via thread-safe, decoupled queue execution.
...
![SeqAnalyzer Dashboard](assets/Dashboard_working.png)
* Once processing begins, the background thread extracts key genomic metrics (such as exact nucleotide distribution, GC-content, and melting temperatures) while you navigate the application.
